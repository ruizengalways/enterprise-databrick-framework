...
def _require_non_empty_string(spec: TableSpec, rule: ReconciliationRule, option_name: str) -> str:
    value = rule.options.get(option_name)
    if not isinstance(value, str) or not value.strip():
        raise ValueError(
            f"{spec.dataset_id}: reconciliation rule {rule.name} requires "
...
def validate_reconciliation_contract(spec: TableSpec) -> None:
    ...
    for rule in spec.reconciliation.rules:
        ...
        if rule.kind == "aggregate":
            _require_non_empty_string(spec, rule, "expression")

        if rule.kind == "operation_count":
            _require_non_empty_string(spec, rule, "operation_column")
